...
Download the full proophboard workspace into local `.proophboard/` JSON files.
...
Write the result array to `.proophboard/chapters/chapters.json` (overwrite if exists).

### 3. Download each chapter
For each chapter in the list, call `mcp__proophboard__get_chapter` with `workspace_id` and `chapter_id`.
Write each full chapter response to `.proophboard/chapters/{chapter_id}.json` (overwrite if exists).

Call `get_chapter` for all chapters in parallel when possible.

### 4. Clean up removed chapters
List existing `.json` files in `.proophboard/chapters/` (excluding `chapters.json`).
...
Write `.proophboard/backup.json` with:
```json
{
