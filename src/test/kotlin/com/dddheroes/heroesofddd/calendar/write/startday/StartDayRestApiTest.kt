...
internal class StartDayRestApiTest @Autowired constructor(val gateways: AxonGatewaysMock) {
    ...
    fun `command success - returns 204 No Content`() {
        gateways.assumeCommandReturns<StartDay>(Success)
        ...
        Given {
            ...
            async().post("/games/{gameId}/calendars/{calendarId}/days")
        } Then {
    ...
    fun `command failure - returns 400 Bad Request`() {
        gateways.assumeCommandReturns<StartDay>(Failure("Cannot skip days"))
        ...
        Given {
            ...
            contentType(ContentType.JSON)
            body("message", equalTo("Cannot skip days"))
        }
    }
}
