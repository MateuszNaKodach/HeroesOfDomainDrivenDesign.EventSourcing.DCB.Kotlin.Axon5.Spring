...
internal class ViewCurrentDaySpringSliceTest @Autowired constructor(
        ...
        fixture.Given {
            ...
            awaitAndExpect { cfg ->
                ...
                assertThat(result).isEqualTo(
                    ViewCurrentDay.Result(month = 1, week = 1, day = 1, finished = false)
                )
            }
        }
    }
    ...
    fun `given multiple days started, then show the latest day`() {
        fixture.Given {
            ...
            awaitAndExpect { cfg ->
                val result = queryCurrentDay(cfg)
                assertThat(result).isEqualTo(
                    ViewCurrentDay.Result(month = 1, week = 1, day = 2, finished = false)
                )
            }
        }
    }
    ...
    fun `given day started and finished, then show that day as finished`() {
        fixture.Given {
            ...
            awaitAndExpect { cfg ->
                val result = queryCurrentDay(cfg)
                assertThat(result).isEqualTo(
                    ViewCurrentDay.Result(month = 1, week = 1, day = 1, finished = true)
                )
            }
        }
    }
    ...
    private fun queryCurrentDay(cfg: Configuration): ViewCurrentDay.Result? =
        cfg.getComponent(QueryGateway::class.java)
            .query(ViewCurrentDay(gameId), ViewCurrentDay.Result::class.java)
