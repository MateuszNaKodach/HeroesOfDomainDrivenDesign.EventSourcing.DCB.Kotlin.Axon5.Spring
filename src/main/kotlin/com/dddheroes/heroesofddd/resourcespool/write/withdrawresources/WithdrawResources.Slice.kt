...
@Command(namespace = "ResourcesPool", name = "WithdrawResources", version = "1.0.0")
data class WithdrawResources(
...
private fun decide(command: WithdrawResources, state: State): List<HeroesEvent> {
    if (command.resources !in state.balance) {
        throw IllegalStateException("Cannot withdraw more than deposited resources")
    }
...
@ConditionalOnProperty(prefix = "slices.resourcespool", name = ["write.withdrawresources.enabled"])
@EventSourced(tagKey = EventTags.RESOURCES_POOL_ID)
private class WithdrawResourcesEventSourcedState private constructor(val state: State) {

    @EntityCreator
    constructor() : this(initialState)

    @EventSourcingHandler
    fun evolve(event: ResourcesDeposited) = WithdrawResourcesEventSourcedState(evolve(state, event))

    @EventSourcingHandler
    fun evolve(event: ResourcesWithdrawn) = WithdrawResourcesEventSourcedState(evolve(state, event))
}

@ConditionalOnProperty(prefix = "slices.resourcespool", name = ["write.withdrawresources.enabled"])
@Component
private class WithdrawResourcesCommandHandler {

    @CommandHandler
    fun handle(
        command: WithdrawResources,
        metadata: AxonMetadata,
        @InjectEntity(idProperty = EventTags.RESOURCES_POOL_ID) eventSourced: WithdrawResourcesEventSourcedState,
