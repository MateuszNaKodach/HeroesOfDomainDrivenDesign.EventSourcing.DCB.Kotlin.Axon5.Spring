...
private fun decide(command: AddCreatureToArmy, state: State): List<HeroesEvent> {
    val isNewCreatureType = !state.creatureStacks.containsKey(command.creatureId)
    if (isNewCreatureType && state.creatureStacks.size >= MAX_CREATURE_STACKS) {
...
private fun evolve(state: State, event: ArmyEvent): State = when (event) {
    ...
    is CreatureRemovedFromArmy -> {
        ...
        val updatedStacks = if (newQuantity == Quantity.zero()) {
            state.creatureStacks - event.creatureId
        } else {
            state.creatureStacks + (event.creatureId to newQuantity)
        }
        state.copy(creatureStacks = updatedStacks)
    }
}
...
@EventSourced(tagKey = EventTags.ARMY_ID) // ConsistencyBoundary
